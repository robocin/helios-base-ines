...
int Bhv_PenaltyKick::M_idx_penalty;
...
void Bhv_PenaltyKick::definePenaltyOrder() {
    for (int i = 0; i < 11; i++) {
...
bool Bhv_PenaltyKick::doGetBall(PlayerAgent* agent) {
 ...
 */
bool Bhv_PenaltyKick::doOneKickShoot(PlayerAgent* agent) {
 ...
 */
bool Bhv_PenaltyKick::getShootTarget(const PlayerAgent* agent,
                                     Vector2D* point,
                                     double* first_speed) {
    const WorldModel& wm = agent->world();
...
*/
Vector2D Bhv_PenaltyKick::getGoalieMovePos(const Vector2D& ball_pos, const Vector2D& my_pos) {
    const ServerParam& SP = ServerParam::i();
 ...
 */
bool Bhv_PenaltyKick::doGoalieSlideChase(PlayerAgent* agent) {
    const WorldModel& wm = agent->world();
